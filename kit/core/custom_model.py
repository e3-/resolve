...
attribute_units = pd.read_csv(pathlib.Path(__file__).parents[1] / "core" / "units.csv")


def get_units(attr_name: str):
    return units.Quantity(
        attribute_units.loc[attribute_units["attribute"] == attr_name, "unit"].values[0]
    )
...
def format_unit(unit, registry, **kwargs):
    string = ""
    if (
        unit == units.dollar / units.kW_year
        or unit == units.dollar / units.kilowatt / units.year
    ):
        return "$/kW⋅year"
    elif (
        unit == units.dollar / units.MWh
        or unit == units.dollar / units.megawatt / units.hour
    ):
        return "$/MWh"
    elif (
        unit == units.dollar / units.kWh
        or unit == units.dollar / units.kilowatt / units.hour
    ):
        return "$/kWh"
    elif (
        unit == units.dollar / units.MMBtu_year
        or unit == units.dollar / units.MMBtu / units.year
    ):
        return "$/MMBtu⋅year"
    elif (
        unit == units.dollar / units.metric_ton_year
        or unit == units.dollar / units.metric_ton / units.year
    ):
