...
class Timeseries(custom_model.BaseCustomModel):
    ...
    def dict(self, **kwargs):
        ...
        return super(Timeseries, self).dict(
            exclude=attrs_to_exclude, exclude_defaults=True, exclude_none=True
        )
    ...
    def infinity(cls):
        return cls(
            name="infinities", data=pd.Series({pd.to_datetime("1/1/1900"): np.inf})
        )
    ...
    def validate_or_convert_to_series(cls, values):
        ...
        if len(values["data"]) == 0:
            ...
            try:  # See if the data is a float
                values["data"] = pd.Series(
                    {pd.Timestamp("1/1/1900"): float(values["data"])}
                )
            except (
                ValueError
            ):  # If it seems like a string, then try to parse as a filepath
                # Deal with users using "\" instead of "/" as filepath separator
                if "\\" in values["data"]:
                    regularized_filepath = values["data_dir"] / pathlib.Path(
                        pathlib.PureWindowsPath(values["data"])
                    )
                else:
                    regularized_filepath = values["data_dir"] / pathlib.Path(
                        values["data"]
                ...
                if regularized_filepath.exists():
                    ...
                    raise FileNotFoundError(
                        f"Cannot find filepath to {values['data']}. Try using an absolute path."
                    )
            values["data"] = (
    ...
    def from_annual_series(cls, name: str, data: pd.Series, **kwargs):
        ...
        if not (data.index.astype(int) >= 1000).all():
            raise ValueError(
                "Series does not have valid index values for conversion (integers >= 1000)."
            )
        series_dt = data.copy()
        series_dt.index = pd.DatetimeIndex(
            [
                pd.Timestamp(year=year, month=1, day=1)
                for year in series_dt.index.astype(int)
            ],
    ...
    def from_dir(cls, directory, filetype="csv") -> Dict[str, "Timeseries"]:
        ...
        if filetype not in ["csv", "json"]:
            raise Exception(
                f"Argument 'filetype' = {filetype} not valid; file_types must be either 'csv' or 'json'"
            )
        ...
        for i, f in enumerate(files):
            ...
            if filetype == "csv":
                ...
                raise ValueError(
                    f"File type must be [csv, json]. File type provided was '{filetype}'"
                )
    ...
    def days_in_year(self) -> pd.Series:
        ...
        days_in_year = pd.Series(
            data=[366 if leap_year else 365 for leap_year in is_leap_year],
    ...
    def to_json(self):
        ...
        )  # Will create processed directory if it does not exist
    ...
    def slice_by_timepoint(self, temporal_settings, model_year, period, hour):
        ...
        if self.weather_year:
            ...
            chrono_dt = pd.Timestamp(
                f"{model_year}-{chrono_dt.month}-{chrono_dt.day} {chrono_dt.hour}:00:00"
            )
        ...
        if self.type == TimeseriesType.MONTHLY:
            ...
            chrono_dt = (
                f"{month_to_season[chrono_dt.month]}-01 {chrono_dt.hour:02d}:00:00"
            )
    ...
    def add_leap_day(self, year, interval):
        ...
        if not len(self.data.loc[year]) > (365 * 24 * 60 / interval):
            ...
            profile_add_day = self.data.loc[year][
                int((31 + 27) * 24 * 60 / interval) : int(
                    (31 + 28) * 24 * 60 / interval
                )
            ]
    ...
    def resample_up(df, frequency, method):
        ...
        if method == "interpolate":
            ...
            raise ValueError(
                f"Unsupported argument for resample_up(): method=`{method}`"
            )
    ...
    def resample_down(df, frequency, method):
        ...
        if method == "sum":
            ...
            raise ValueError(
                f"Unsupported argument for resample_down(): method=`{method}`"
            )
    ...
    def resample_month_or_season_hour_to_hourly(self, correct_index: pd.DatetimeIndex):
        ...
        if self.type == TimeseriesType.MONTH_HOUR:
            ...
            new_profile = pd.Series(
                name=self.data.name, index=correct_index, data=np.nan
            )
    ...
    def resample_simple_extend_years(self, weather_years: tuple[int, int]):
        ...
        drange = pd.date_range(
            f"01/01/{first_weather_year} 00:00",
            f"12/31/{last_weather_year} 23:00",
        ...
        for year in range(first_weather_year, last_weather_year + 1):
            ...
            elif len(year_index) > len(
                raw
            ):  # if weather year is a leap year and model year isn't
                new.loc[year_index[: len(raw)]] = (
                    raw.values
                )  # put the values in for first 8760
                new.loc[year_index[len(raw) :]] = raw.values[
    ...
    def repeat_ts(self, repeat_year_dict):
        ...
        for year in years:
            ...
            if self.freq in ["H", "D"]:
                if calendar.isleap(year) and not calendar.isleap(
                    repeat_year_dict[year]
                ):
                    # Add leap day
                    data_repeated_year_by_dayofyear = data_repeated_year.groupby(
                        data_repeated_year.index.dayofyear
                    )
                    dayofyear_order = list(range(1, 366))
                    dayofyear_order.insert(59, 59)  # Add leap day (repeat Feb 28)
                    data_repeated_year = pd.concat(
                        [
                            data_repeated_year_by_dayofyear.get_group(dayofyear)
                            for dayofyear in dayofyear_order
                        ],
                        axis=0,
                    )
                elif not calendar.isleap(year) and calendar.isleap(
                    repeat_year_dict[year]
                ):
                    # Remove leap day
                    data_repeated_year_by_dayofyear = data_repeated_year.groupby(
            ...
            data_repeated = pd.concat(
                [data_repeated, data_repeated_year], axis=0, ignore_index=True
            )
...
class BaseBooleanTimeseries(Timeseries):
    @field_validator("data")
        ...
        if data.dtype != bool:
            ...
            with pd.option_context("future.no_silent_downcasting", True):
                data = (
                    data.astype("object")
                    .replace(
                        [1, 1.0, "1", "1.0", True, "TRUE", "True", "true", "T", "t"],
                        True,
                    )
                    .infer_objects(copy=False)
                )
                data = data.replace(
                    [0, 0.0, "0", "0.0", False, "FALSE", "False", "false", "F", "f"],
                    False,
                ).infer_objects(copy=False)

                # Fillna as necessary
                data = data.ffill().astype(bool)

        # If after replacing values, the Series is still not Boolean, raise error
        assert (
            data.dtype == bool
        ), f"Timeseries data for {values.data['name']} should be boolean (True/False)"
        return data
