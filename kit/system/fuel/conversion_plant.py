...
from kit.common.asset.plant import Plant
...
class FuelConversionPlant(Plant):
    ...
    def opt_annual_fuel_consumption_all_fuels_mmbtu(self) -> ts.NumericTimeseries:
        if self.opt_annual_fuel_consumption_by_fuel_mmbtu is None:
            ...
            annual_consumption_all_fuels = (
                self.opt_annual_fuel_consumption_by_fuel_mmbtu.groupby(
                    ["MODEL_YEARS"]
                ).sum()
            )
            annual_consumption_all_fuels.index = pd.to_datetime(
                annual_consumption_all_fuels.index, format="%Y"
            )
            annual_consumption_all_fuels = ts.NumericTimeseries(
                name="opt_annual_fuel_consumption_all_fuels_mmbtu",
                data=annual_consumption_all_fuels,
            )
        ...
        if candidate_fuel == self.produced_candidate_fuel_name:
            production_in_timepoint_mmbtu = self.conversion_efficiency * sum(
                model.Fuel_Conversion_Plant_Consumption_In_Timepoint_MMBTU[
                    self.name, cf, model_year, rep_period, hour
                ]
    ...
    def Fuel_Conversion_Plant_Variable_Cost_In_Timepoint(
