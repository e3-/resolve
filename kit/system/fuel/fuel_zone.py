...
class FuelZone(component.BaseComponent):
    ...
    def fuel_production_plants(self):
        if self.electrolyzers is None and self.fuel_conversion_plants is None:
            ...
            fuel_production_plants = (self.electrolyzers or dict()) | (
                self.fuel_conversion_plants or dict()
            )
    ...
    def Net_Candidate_Fuel_Imports_In_Timepoint_MMBTU_H(
        ...
        if candidate_fuel in self.candidate_fuels.keys():
            for fuel_transportation in resolve_case.model.FUEL_TRANSPORTATIONS:
                if (
                    resolve_case.system.fuel_transportations[
                        fuel_transportation
                    ].unique_candidate_fuel
                    == candidate_fuel
                ):
                    if (
                        resolve_case.model.fuel_transportation_to[fuel_transportation]
                        == self.name
                    ):
                        imports_exports += (
                            resolve_case.model.Transmit_Candidate_Fuel_MMBTU_H[
                                fuel_transportation, model_year, rep_period, hour
                            ]
                        )
                    elif (
                        resolve_case.model.fuel_transportation_from[fuel_transportation]
                        == self.name
                    ):
                        imports_exports -= (
                            resolve_case.model.Transmit_Candidate_Fuel_MMBTU_H[
    ...
    def Fuel_Zone_Candidate_Fuel_Production_In_Timepoint_MMBTU_H(
        ...
        if self.fuel_production_plants is None or len(self.fuel_production_plants) == 0:
            ...
            return sum(
                ...
                if candidate_fuel
                in self.fuel_production_plants[
                    fuel_production_plant
                ].instance_from.candidate_fuels.keys()
            )
    ...
    def Fuel_Zone_Candidate_Fuel_Storage_In_Timepoint_MMBTU_H(
        ...
        if self.fuel_storage_instances is not None:
            for fuel_storage in self.fuel_storage_instances.keys():
                ...
                if (
                    candidate_fuel
                    in self.fuel_storages[
                        fuel_storage
                    ].instance_from.candidate_fuels.keys()
                ):
                    storage += model.Fuel_Storage_Charging_MMBtu_per_Hr[
                        fuel_storage, model_year, rep_period, hour
                    ]

                    storage -= model.Fuel_Storage_Discharging_MMBtu_per_Hr[
                        fuel_storage, model_year, rep_period, hour
                    ]
        return storage
    ...
    def Fuel_Zone_Candidate_Fuel_Constraint(
        self, model, candidate_fuel, model_year, rep_period, hour
    ):
        """
        ...
        return (
            ...
            - model.Unserved_Energy_MMBTU_H[
                self.name, candidate_fuel, model_year, rep_period, hour
            ]
            + model.Overproduction_MMBTU_H[
                self.name, candidate_fuel, model_year, rep_period, hour
            ]
    ...
    def Annual_Fuel_Zone_Candidate_Fuel_Consumption_For_Final_Fuel_Demands_MMBTU(
        ...
        return sum(
            ...
            for final_fuel_demand in model.FINAL_FUEL_DEMANDS
            if candidate_fuel
            in model.FINAL_FUEL_DEMAND_CANDIDATE_FUELS[final_fuel_demand]
        )

    def Annual_Fuel_Zone_Candidate_Fuel_Balance(
        self, model, candidate_fuel, model_year
    ):
        return (
            model.Annual_Fuel_Zone_Candidate_Fuel_Production_MMBTU[
                self.name, candidate_fuel, model_year
            ]
            + model.Annual_Net_Candidate_Fuel_Imports_MMBTU[
                self.name, candidate_fuel, model_year
            ]
            - model.Annual_Net_Fuel_Zone_Candidate_Fuel_Consumption_MMBTU[
                self.name, candidate_fuel, model_year
            ]
    ...
    def Satisfy_Fuel_Zone_Final_Fuel_Hourly_Demands_Constraint(
        ...
        if self.final_fuels[final_fuel_demand].instance_from.annual_demand is None:
            ...
            constraint = sum(
                ...
                for candidate_fuel in model.FINAL_FUEL_DEMAND_CANDIDATE_FUELS[
                    final_fuel_demand
                ]
            ) == self.final_fuels[
                final_fuel_demand
            ].instance_from.annual_demand.slice_by_timepoint(
    ...
    def Satisfy_Fuel_Zone_Final_Fuel_Annual_Demands_Constraint(
        self, model, final_fuel_demand, model_year
    ):
        """
        ...
        if self.final_fuels[final_fuel_demand].instance_from.annual_demand is None:
            ...
            constraint = sum(
                ...
                final_fuel_demand
            ].instance_from.annual_demand.slice_by_year(
                model_year
            )
    ...
    def electrolyzer_instances(self) -> Dict[str, Electrolyzer]:
        electrolyzers = (
            {
                name: linkage.instance_from
                for name, linkage in self.electrolyzers.items()
            }
    ...
    def fuel_storage_instances(self) -> Dict[str, FuelStorage]:
        fuel_storages = (
            {
                name: linkage.instance_from
                for name, linkage in self.fuel_storages.items()
            }
        ...
        paths_to = (
            {
                ...
                for name, linkage in self.fuel_transportations.items()
                if linkage.to_zone
            }
    ...
    def opt_annual_imports_mmbtu(self) -> ts.NumericTimeseries:
        if (
            self.fuel_transportation_instances_to_zone is None
            and self.fuel_transportation_instances_from_zone is None
        ):
            total_imports = None
        else:
            forward_imports = sum(
            ...
            if isinstance(total_imports, int) and total_imports == 0:
                ...
                total_imports = ts.NumericTimeseries(
                    name="opt_total_annual_imports_mmbtu", data=total_imports
                )
    ...
    def opt_annual_exports_mmbtu(self) -> ts.NumericTimeseries:
        if (
            ...
            if isinstance(total_exports, int) and total_exports == 0:
                ...
                total_exports = ts.NumericTimeseries(
                    name="opt_total_annual_exports_mmbtu", data=total_exports
                )
    ...
    def opt_annual_net_imports_mmbtu(self) -> ts.NumericTimeseries:
        if (
            self.opt_annual_imports_mmbtu is None
            and self.opt_annual_exports_mmbtu is None
        ):
            net_imports = None
        elif self.opt_annual_exports_mmbtu is None:
            net_imports = ts.NumericTimeseries(
                name="opt_net_annual_imports_mmbtu",
                data=self.opt_annual_imports_mmbtu.data,
            )
        elif self.opt_annual_imports_mmbtu is None:
            net_imports = ts.NumericTimeseries(
                name="opt_net_annual_exports_mmbtu",
                data=self.opt_annual_exports_mmbtu.data,
            )
        else:
            net_imports = ts.NumericTimeseries(
                name="opt_annual_imports_mmbtu",
                data=self.opt_annual_imports_mmbtu.data
                - self.opt_annual_exports_mmbtu.data,
            )
        return net_imports
