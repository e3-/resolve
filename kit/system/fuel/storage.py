...
from kit.common.asset import plant
...
class FuelStorage(plant.Plant):
    ...
    fuel_storage_duration: Optional[PositiveFloat] = Field(
        ...
        "Exclusive of "
        ":py:attr:`kit.fuel_system.fuel_storage.FuelStorage.planned_storage_capacity`.",
        default_freq="YS",
        up_method="ffill",
        down_method="annual",
    )
    planned_storage_capacity: Optional[ts.NumericTimeseries] = Field(
        ...
        "Exclusive of "
        ":py:attr:`kit.fuel_system.fuel_storage.FuelStorage.duration`.",
        default_freq="YS",
        up_method="ffill",
        down_method="annual",
    )
    ...
    def validate_storage_duration(cls, values):
        if (
            values["fuel_storage_duration"] is not None
            and values["planned_storage_capacity"] is not None
        ):
            raise ValueError(
    ...
    # Cost Attributes #
    ###################
    planned_fuel_storage_capacity_fixed_om_by_model_year: Optional[
    ...
    new_fuel_storage_capacity_fixed_cost_dollars_per_mmbtu_yr_by_vintage: Optional[
        ts.NumericTimeseries
    ] = Field(
        None,
    ...
    new_fuel_storage_fixed_om_by_vintage: Optional[ts.NumericTimeseries] = Field(
        ...
        ":py:attr:`kit.system.fuel.fuel_storage.FuelStorage.new_fuel_storage_capacity_fixed_cost_dollars_per_mmbtu_yr_by_vintage`",
        default_freq="YS",
        up_method="interpolate",
        down_method="annual",
    )
    ...
    ############################
    def Increase_Load_For_Charging_Fuel_Storage_MW(
        self, model, model_year, rep_period, hour
    ):
        return (
            model.Fuel_Storage_Charging_MMBtu_per_Hr[
                self.name, model_year, rep_period, hour
            ]
            * self.fuel_storage_charging_efficiency_mwh_per_mmbtu
        )

    def Increase_Load_For_Discharging_Fuel_Storage_MW(
        self, model, model_year, rep_period, hour
    ):
        return (
            model.Fuel_Storage_Discharging_MMBtu_per_Hr[
                self.name, model_year, rep_period, hour
            ]
            * self.fuel_storage_charging_efficiency_mwh_per_mmbtu
        )
    ...
    def Fuel_Storage_Consumption_Increase_Load_Capacity_In_Timepoint_MW(
        ...
        if self.fuel_storage_charging_efficiency_mwh_per_mmbtu is not None:
            ...
            return (
                model.Operational_Capacity_In_Model_Year[
                    self.name, model, model_year, rep_period, hour
                ]
                * 0.0
            )
    ...
    def Fuel_Storage_Production_Increase_Load_Capacity_In_Timepoint_MW(
        ...
        if self.fuel_storage_discharging_efficiency_mwh_per_mmbtu is not None:
            ...
            return (
                model.Operational_Fuel_Storage_Volume_In_Model_Year[
                    self.name, model, model_year, rep_period, hour
                ]
                * 0.0
            )

    def Fuel_Storage_SOC_Inter_Intra_Joint(
        self, model, model_year, chrono_period, hour, rep_period
    ):
        """
    ...
    def Fuel_Storage_Discharging_Max_Constraint(
        self, model, model_year, rep_period, hour
    ):
        return (
            model.Fuel_Storage_Discharging_MMBtu_per_Hr[
    ...
    def Fuel_Storage_Duration_Constraint(self, model, model_year):
        if self.fuel_storage_duration is None:
            ...
            return (
                model.Operational_Fuel_Storage_Volume_In_Model_Year[
                    self.name, model_year
                ]
                == model.Operational_Capacity_In_Model_Year[self.name, model_year]
                * self.fuel_storage_duration
            )

    def Fuel_Storage_Increase_Load_Constraint(
        self, model, model_year, rep_period, hour
    ):
        return (
    ...
    def Fuel_Storage_SOC_Inter_Intra_Max_Constraint(
        self, model, model_year, chrono_period, hour
    ):
        """
        ...
        """
        return (
            model.Fuel_Storage_SOC_Inter_Intra_Joint[
                self.name, model_year, chrono_period, hour
            ]
            <= model.Operational_Fuel_Storage_Volume_In_Model_Year[
                self.name, model_year
            ]
    ...
    def Fuel_Storage_SOC_Inter_Intra_Min_Constraint(
        self, model, model_year, chrono_period, hour
    ):
        """
    ...
    def Fuel_Storage_SOC_Intra_Anchoring_Constraint(
        self, model, model_year, rep_period, hour
    ):
        if (
            self.allow_inter_period_sharing
            and (model_year, rep_period, hour)
            == model.first_timepoint_of_period[model_year, rep_period]
        ):
            return (
    ...
    def Fuel_Storage_SOC_Intra_Tracking_Constraint(
        ...
        if (model_year, rep_period, hour) == model.last_timepoint_of_period[
            model_year, rep_period
        ]:
            constraint = pyo.Constraint.Skip
        else:
            charged_mmbtu = model.Fuel_Storage_Charging_MMBtu_per_Hr[
                self.name, model_year, rep_period, hour
            ]

            discharged_mmbtu = model.Fuel_Storage_Discharging_MMBtu_per_Hr[
                self.name, model_year, rep_period, hour
            ]
    ...
    def Fuel_Storage_SOC_Inter_Tracking_Constraint(
        ...
        constraint = (
            model.Fuel_Storage_SOC_Inter_Period[
                self.name, model_year, next_chrono_period
            ]
            == self._apply_parasitic_loss(
                model.Fuel_Storage_SOC_Inter_Period[
                    self.name, model_year, chrono_period
                ],
                self.fuel_storage_parasitic_loss,
            ...
            - discharged_mmbtu
            - model.Fuel_Storage_SOC_Intra_Period[
                self.name, model_year, rep_period, first_hour
            ]
        )
    ...
    def Fuel_Storage_Simultaneous_Charging_Constraint(
        self, model, model_year, rep_period, hour
    ):
        """
        ...
        return (
            model.Increase_Load_For_Charging_Fuel_Storage_MW[
                self.name, model_year, rep_period, hour
            ]
            + model.Increase_Load_For_Discharging_Fuel_Storage_MW[
                self.name, model_year, rep_period, hour
            ]
            <= model.Plant_Increase_Load_Capacity_In_Timepoint_MW[
                self.name, model_year, rep_period, hour
            ]
        )
    ...
    opt_increase_load_charging_mw: Optional[pd.Series] = Field(
        None,
        description="Optimized gross consumption by timepoint for charging storage reservoir, in MW.",
    )
    opt_increase_load_discharging_mw: Optional[pd.Series] = Field(
        None,
        description="Optimized gross consumption by timepoint for discharging storage reservoir, in MW.",
    )
    ...
    def opt_total_operational_storage_capacity(self):
        total_operational_capacity = self.sum_timeseries_attributes(
            attributes=[
                "opt_operational_planned_storage_capacity",
    ...
    def opt_retired_storage_capacity(self):
        if self.planned_storage_capacity is None:
            ...
            return ts.NumericTimeseries(
                ...
                data=self.planned_storage_capacity.data
