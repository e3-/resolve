...
class FuelTransportation(asset.BaseAsset):
    ...
    def revalidate(self):
        ...
        if len(to_zones) > 1:
            raise ValueError(
                f"Multiple zones `{to_zones}` are marked as being on the 'to' side of path `{self.name}`."
            )
        elif len(to_zones) == 0:
    ...
    def Fuel_Transportation_Max_Flow_Constraint(
        ...
        return model.Transmit_Candidate_Fuel_MMBTU_H[
            ...
            model.Operational_Capacity_In_Model_Year[self.name, model_year]
            * self.forward_flow_profile.slice_by_timepoint(
                temporal_settings, model_year, rep_period, hour
            )
        )
    ...
    def Fuel_Transportation_Transmit_Candidate_Fuel_Rule_Forward(
        ...
        return (
            model.Transmit_Candidate_Fuel_Forward_MMBTU_H[
                self.name, model_year, rep_period, hour
            ]
            >= model.Transmit_Candidate_Fuel_MMBTU_H[
                self.name, model_year, rep_period, hour
            ]
        )
    ...
    def Fuel_Transportation_Transmit_Candidate_Fuel_Rule_Reverse(
        ...
        return (
            model.Transmit_Candidate_Fuel_Reverse_MMBTU_H[
                self.name, model_year, rep_period, hour
            ]
            >= -model.Transmit_Candidate_Fuel_MMBTU_H[
                self.name, model_year, rep_period, hour
    ...
    def Fuel_Transportation_Hurdle_Cost_In_Timepoint_Forward(
        ...
        return model.Transmit_Candidate_Fuel_Forward_MMBTU_H[
            self.name, model_year, rep_period, hour
    ...
    def Fuel_Transportation_Hurdle_Cost_In_Timepoint_Reverse(
        ...
        return model.Transmit_Candidate_Fuel_Reverse_MMBTU_H[
            self.name, model_year, rep_period, hour
    ...
    def unique_candidate_fuel(self):
        if self.candidate_fuels:
            ...
            if len(candidate_fuels) > 1:
                ...
                raise ValueError(
                    f"No candidate fuels assigned to FuelTransportation '{self.name}'"
                )
            else:
