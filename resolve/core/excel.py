...
class ComponentToWrite(BaseCustomModel):
    ...
    def construct(
        ...
        *,
        ...
        component = getattr(importlib.import_module(f"resolve.{module_name}"), class_name)
...
class ScenarioTool(ExcelTemplate):
    ...
    def export_system(
        ...
        for (module_name, class_name), tables in sorted(component_tables.items(), key=lambda item: item[0][1]):
            ...
            try:
                component_class = getattr(importlib.import_module(f"resolve.{module_name}"), class_name)
    ...
    def _format_attributes_dataframe(self, data):
        # Update 'include' column to nullable boolean so it stays True/False/NaN instead of numeric
        data[[col for col in data.columns if col[0] == "include"]] = data[
            [col for col in data.columns if col[0] == "include"]
        ].astype("boolean")
