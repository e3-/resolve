...
class Linkage(Component):
    ...
    instance_from: Component | None = None  # Not ideal, since a linkage should only exist if there are linked instances
    instance_to: Component | None = None
    # Filename attributes
    ...
    def _filter_scenarios(cls, linkages_df: pd.DataFrame, scenarios: List[str], filepath: pathlib.Path) -> pd.DataFrame:
        ...
        grouped_linkages_df = linkages_df.groupby(group_columns, as_index=False)[linkages_df.columns]
...
class CandidateFuelToResource(Linkage):
    """Linkage between resolve.common.fuel.CandidateFuel and resolve.common.resource.Resource.
...
class _AllToPolicy(Linkage):
    ...
    def _get_component_type_to_(self):
        from resolve.system import policy
