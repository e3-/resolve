...
class TemporalSettings(FromCSVMixIn):
    ...
    def from_dir(cls, temporal_settings_dir: pathlib.Path) -> "TemporalSettings":
        ...
        if chrono_period_file.exists():
            ...
            with pd.option_context("future.no_silent_downcasting", True):
                chrono_periods_map = chrono_periods_map.infer_objects(copy=False)
    ...
    def _calculate_dispatch_window_weights(self):
        ...
        if self.chrono_periods_map is not None:
            ...
            with pd.option_context("future.no_silent_downcasting", True):
                self.chrono_periods_map.index = self.chrono_periods_map.index.infer_objects(copy=False)
                self.chrono_periods_map = self.chrono_periods_map.infer_objects(copy=False)
    ...
    def _calculate_discount_factors(self) -> ts.NumericTimeseries:
        ...
        df["compounding_discount_rate"] = df["compounding_discount_rate"].ffill()
        ...
        # 5. Sum up end effect years, escalating final modeled-year dollars into represented future years.
        tail_mask = df.index >= modeled_year_end
        if tail_mask.any():
            tail_years = df.index[tail_mask]
            tail_exponent = (tail_years.year - modeled_year_end.year).astype(int)
            tail_escalation = pd.Series((1.0 + inflation_rate) ** tail_exponent, index=tail_years)

            df.loc[modeled_year_end, "compounding_discount_rate"] = (
                df.loc[tail_years, "compounding_discount_rate"] * tail_escalation
            ).sum()
        ...
        # Convert modeled-year costs into each represented calendar year's dollars before applying nominal discounting.
        df["inflation_from_start_year"] = (1.0 + inflation_rate) ** (df.index.year - df["start_year"].dt.year)

        df["inflation_from_end_year"] = (1.0 + inflation_rate) ** (df.index.year - df["end_year"].dt.year)

        # 7. Weight compounding discount rate by ``start_year_weight`` and ``end_year_weight``
        df["end_year_weight"] = (
            df["end_year_weight"]
            / df["num_years_between_modeled"]
            * df["compounding_discount_rate"]
            * df["inflation_from_end_year"]
        )
        df["start_year_weight"] = (
            df["start_year_weight"]
            / df["num_years_between_modeled"]
            * df["compounding_discount_rate"]
            * df["inflation_from_start_year"]
        )
