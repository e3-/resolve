...
class Timeseries(custom_model.BaseCustomModel):
    ...
    def validate_or_convert_to_series(cls, values):
        ...
        if len(values["data"]) == 0:
            ...
            try:  # See if the data is a float
                ...
                values["data"] = (
                    pd.read_csv(path, index_col=0, parse_dates=True, date_format="mixed").dropna(axis=1).squeeze(axis=1)
                )
    ...
    def resample_up(df, method):
        ...
        with pd.option_context("future.no_silent_downcasting", True):
            if method == "interpolate":
                df = df.interpolate().infer_objects(copy=False)
            elif method == "ffill":
    ...
    def modeled_year_hourly_ts_to_dict(self, modeled_years: tuple[int, int], weather_years: tuple[int, int]) -> dict:
        """Resample hourly modeled year multiplier with simple extend years function

        Args:
            modeled_years: tuple of first and last modeled year
            weather_years: tuple of first and last weather year

        Returns:
            hourly_ts_by_modeled_year_dict: dictionary of WY-indexed Timeseries objects for each modeled year

        """
        first_modeled_year, last_modeled_year = modeled_years
        hourly_ts_by_modeled_year_dict = {}
        for year in range(first_modeled_year, last_modeled_year + 1):
            ts_for_this_year = copy.deepcopy(self)
            ts_for_this_year.data = ts_for_this_year.data.loc[ts_for_this_year.data.index.year == year]
            ts_for_this_year.resample_simple_extend_years(weather_years=weather_years)
            hourly_ts_by_modeled_year_dict[year] = ts_for_this_year

        return hourly_ts_by_modeled_year_dict
...
class BooleanTimeseries(Timeseries, BaseBooleanTimeseries):
    """Timeseries subclass for boolean data."""
