...
from resolve.system.electric.resources import ElectricResourceGroup
...
class ResolveModel(ModelTemplate):
    ...
    def export_results_summary(self, output_dir: pathlib.Path, results_reporting: dict):
        ...
        for component in self.system._optimization_construction_order:
            ...
            elif not slack_costs_df.empty:
                for col in slack_cost_summary.columns:
                    dtype = slack_cost_summary[col].dtype
                    slack_costs_df[col] = slack_costs_df[col].astype(dtype)
                slack_cost_summary = pd.concat([slack_cost_summary, slack_costs_df], ignore_index=False)
