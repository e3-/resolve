...
import resolve.resolve.config as config
from resolve import __version__
from resolve.core import stream
...
# Configure default loguru logger (stderr)
def _bootstrap_logging(log_level: str = "INFO", log_json: bool = False):
    logger.remove()
    logger.add(sys.__stdout__, level=log_level, serialize=log_json)


def get_objective_function_value(instance, output_dir: Path):
...
def _run_case(
    ...
    if extras is not None and extras.strip():
        plugin_modules = importlib.import_module(f"resolve.resolve.extras.{extras.strip()}")
...
def case_main(
    ...
    if config.ENABLE_RAY:
        _bootstrap_logging(log_level, log_json=False)

    logger.info(f"Loading Resolve case: {case_name}")

    # Make folders
    dir_str.make_resolve_dir(resolve_settings_name=case_name)
...
def main(
    ...
    data_folder: str = typer.Option(
        "data", help="Name of data folder, which is assumed to be in the same folder as top level `resolve` folder."
    ),
    ...
    cpus_per_task: float = typer.Option(1.0, help="Number of CPUs to allocate per task when running with Ray."),
    memory_per_task_bytes: int = typer.Option(
        9_000_000_000, help="Amount of memory to allocate per task when running with Ray, in bytes."
    ),
) -> Optional[list[ResolveModel]]:
    _bootstrap_logging(log_level, log_json)
    ...
    logger.info(f"cpus_per_task = {cpus_per_task}")
    logger.info(f"memory_per_task_bytes = {memory_per_task_bytes}")

    # Create folder for the specific resolve run
    dir_str = (
        DirStructure(start_dir=Path.cwd(), data_folder=data_folder)
        if config.RUNNING_ON_ANYSCALE
        else DirStructure(data_folder=data_folder)
    )
    ...
    resolve_cases = []
    resolve_case_handles = []
    for resolve_settings_name in cases_to_run:
        if config.ENABLE_RAY:
            # Dispatch case to run on Ray cluster
            handle = (
                # TODO: How do we use memory here? Should we use config.MAIN_MEMORY or memory_per_task_bytes?
                ray.remote(case_main)
                .options(num_cpus=cpus_per_task, max_retries=0, memory=config.MAIN_MEMORY)
                .remote(
            ...
            resolve_case_handles.append(handle)
        else:
            resolve_model = case_main(
                case_name=resolve_settings_name,
                dir_str=dir_str,
                extras=extras,
                solver_name=solver_name,
    ...
    # Wait for remote cases to complete and gather results,
    # this needs to be called as otherwise ray will not execute the tasks.
    if config.ENABLE_RAY:
        resolve_cases = ray.get(resolve_case_handles)
