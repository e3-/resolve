...
class Reserve(component.Component):
    ...
    def _construct_output_expressions(self, construct_costs: bool):
        """Construct reserve price output expressions.

        Args:
            construct_costs: Whether cost-related output expressions should be constructed.
        """
        if construct_costs:
            self.formulation_block.hourly_reserve_price_weighted = pyo.Expression(
                self.formulation_block.model().MODELED_YEARS,
                self.formulation_block.model().DISPATCH_WINDOWS_AND_TIMESTAMPS,
                rule=self._hourly_reserve_price_weighted,
                doc="Hourly Reserve Price Weighted ($/MW)",
            )
            self.formulation_block.hourly_reserve_price_unweighted = pyo.Expression(
                self.formulation_block.model().MODELED_YEARS,
                self.formulation_block.model().DISPATCH_WINDOWS_AND_TIMESTAMPS,
                rule=self._hourly_reserve_price_unweighted,
                doc="Hourly Reserve Price Unweighted ($/MW)",
            )
    ...
    def _hourly_reserve_price_weighted(self, block, modeled_year, dispatch_window, timestamp):
        """Return the weighted reserve price from the balance constraint dual.

        Args:
            block: Pyomo block containing reserve output expressions.
            modeled_year: Modeled year for the reserve price.
            dispatch_window: Dispatch window containing the timestamp.
            timestamp: Timestamp for the reserve price.

        Returns:
            Weighted reserve price dual value, or ``np.nan`` when no dual is available.
        """
        return self.formulation_block.operating_reserve_balance_constraint[
            modeled_year, dispatch_window, timestamp
        ].get_suffix_value("dual", default=np.nan)

    def _hourly_reserve_price_unweighted(self, block, modeled_year, dispatch_window, timestamp):
        """Return the reserve price with temporal weighting factors removed.
        ...
        Returns:
            Reserve price normalized by discount, day, dispatch-window, and timestamp weights.
        """
        weighted_dual_value = block.hourly_reserve_price_weighted[modeled_year, dispatch_window, timestamp]
