...
class FlexLoadResource(BaseFlexLoadResource, ShedDrResource, StorageResource):
    ...
    def _committed_capacity(
        ...
        """Define committed capacity expression as the resource's committed units."""
    ...
    def _max_dr_call_duration_intra_period_constraint(
        ...
        """
        Constrain the maximum duration of a single demand response (DR) call within a dispatch window for power output only.
        This constraint updates the constraint defined in ShedDrResource to account for the separation of power output and power input committed units for FlexLoadResource.
        """
    ...
    def _annual_dr_call_limit_constraint(self, block: pyo.Block, modeled_year: pd.Timestamp):
        """Constrain the annual power output start units to be less than or equal to the product of the maximum annual calls per unit and the number of units of a given year"""
    ...
    def _monthly_dr_call_limit_constraint(self, block: pyo.Block, modeled_year: pd.Timestamp, month: pd.Timestamp):
        """Constrain the monthly power output start units to be less than or equal to the product of the maximum monthly calls per unit and the number of units of a given year"""
        constraint = sum(
    ...
    def _daily_dr_call_limit_constraint(self, block: pyo.Block, modeled_year: pd.Timestamp, day: pd.Timestamp):
        """Constrain the daily power output start units to be less than or equal to the product of the maximum daily calls per unit and the number of units of a given year"""
        constraint = sum(
            block.start_units_power_output[modeled_year, dispatch_period, timestamp]
...
class FlexLoadResourceGroup(BaseFlexLoadResourceGroup, ShedDrResourceGroup, StorageResourceGroup, FlexLoadResource):
    ...
    def _committed_capacity_mw_power_output_ub_constraint(
        ...
        """
        Upper-bound constraint: power output committed capacity cannot exceed the product of
        the resource group's cumulative capacity and the unit commitment indicator.

        - This constraint enforces that if the unit is not committed (committed_units = 0), then committed capacity is zero.
        - When committed, the capacity is limited by the resource group's cumulative capacity.

        Returns: pyo.Constraint: power_output_committed_capacity <= resource group's cumulative capacity * power_output_committed_units
        """
        return (
            block.committed_capacity_mw_power_output[modeled_year, dispatch_window, timestamp]
    ...
    def _committed_capacity_mw_power_input_ub_constraint(
        ...
        Returns: pyo.Constraint: power_input_committed_capacity <= resource group's cumulative capacity * power_input_committed_units
        """
        return (
            block.committed_capacity_mw_power_input[modeled_year, dispatch_window, timestamp]
            <= self.cumulative_potential.data.at[modeled_year]
