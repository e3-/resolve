...
class HybridVariableResource(VariableResource):
    """A hybrid variable resource is a variable resource that is paired with a storage resource. Parent class for hybrid solar and wind resources."""
    ...
    @property
    def hybrid_linkage(self) -> linkage.HybridStorageResourceToHybridVariableResource:
        """Returns the linkage object connecting this hybrid variable resource to its paired hybrid storage resource. Assumes that there is only one such linkage."""
    ...
    def revalidate(self):
        """Validate that the hybrid variable resource is linked to at most one hybrid storage resource."""
        super().revalidate()
...
class HybridStorageResource(StorageResource):
    ...
    def revalidate(self):
        """Validate that the hybrid storage resource is linked to exactly one hybrid variable resource."""
        super().revalidate()
    ...
    @property
    def hybrid_linkage(self) -> linkage.HybridStorageResourceToHybridVariableResource:
        """Returns the linkage object connecting this hybrid storage resource to its paired hybrid variable resource. Assumes that there is only one such linkage."""
    ...
    def paired_variable_resource(self) -> HybridVariableResource:
        """Returns the hybrid variable resource that is paired with this hybrid storage resource. Assumes that there is only one such resource."""
        return self.hybrid_linkage.instance_to

    @property
    def paired_resource_erm_policy_linkage(self):
        """Returns the ERM policy linkage for the hybrid storage resource for accessibility."""
    ...
    def _construct_operational_rules(
        ...
        # Constraints for paired variable resource are already automatically written
