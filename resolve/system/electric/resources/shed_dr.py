...
from resolve.system.electric.resources.unit_commitment import UnitCommitmentMethod
...
class ShedDrResource(BaseShedDrResource, UnitCommitmentResource):
    """ShedDrResources represent resources that shed demand response (or conventional) loads that can be curtailed to provide capacity reductions."""
    ...
    def _annual_dr_call_limit_constraint(self, block: pyo.Block, modeled_year: pd.Timestamp):
        """Constrain the annual start units to be less than or equal to the product of the maximum annual calls per unit and the number of units of a given year"""
    ...
    def _monthly_dr_call_limit_constraint(self, block: pyo.Block, modeled_year: pd.Timestamp, month: pd.Timestamp):
        """Constrain the monthly start units to be less than or equal to the product of the maximum monthly calls per unit and the number of units of a given year"""
        constraint = sum(
    ...
    def _daily_dr_call_limit_constraint(self, block: pyo.Block, modeled_year: pd.Timestamp, day: pd.Timestamp):
        """Constrain the daily start units to be less than or equal to the product of the maximum daily calls per unit and the number of units of a given year"""
        constraint = sum(
            block.start_units[modeled_year, dispatch_period, timestamp]
    ...
    def _max_dr_call_duration_inter_period_constraint(
        ...
        """
        Constrain maximum up time (power output) for shed DR resources, i.e., length of one DR call in terms of hours (across different days with inter-period sharing).
        """
        (
    ...
    def _erm_power_output_max_constraint(
        ...
        """The power output of the resource under ERM dispatch cannot exceed the product of the committed capacity and the maximum potential at a given timepoint."""
        return (
    ...
    def _erm_energy_budget_daily_MWh(self, block: pyo.Block, modeled_year: pd.Timestamp, weather_period: pd.Timestamp):
        """Fractional daily energy budget scaled to model year's capacity, giving budget in MWh. Assumes weather period is one day."""
    ...
    def _erm_dispatch_cost(
        ...
        """Calculates the dispatch cost for shed DR resources under ERM dispatch as the product of the dispatch cost per MWh and the power output
        plus costs for starting and committing units."""
        dispatch_cost = (
    ...
    def _erm_annual_dispatch_cost(self, block: pyo.Block, modeled_year: pd.Timestamp):
        """Calculates the total annual dispatch cost for shed DR resources under ERM dispatch."""
    ...
    def _erm_committed_capacity(
        ...
        """Calculate the committed capacity as the product of the number of committed units and the unit size."""
...
class ShedDrResourceGroup(BaseShedDrResourceGroup, UnitCommitmentResourceGroup, ShedDrResource):
