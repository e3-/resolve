...
class StorageResource(BaseStorageResource, GenericResource):
    """Resource with storage capacity and state-of-charge tracking."""

    _DOCS_URL: ClassVar[str] = (
        "https://docs.ethree.com/projects/kit/en/main/system/electric/resources/storage.html#resolve.system.storage.StorageResource."
    )
    ...
    def planned_storage_capacity(self) -> pd.Series:
        """Returns the planned storage capacity in MWh, calculated as the planned power capacity in MW multiplied by the duration."""
        return self.planned_capacity.data * self.duration

    @property
    def imax_profile(self) -> pd.Series:
        """Returns the Imax profile, which is the maximum power input profile, as a pd.Series. This is used for storage resources to define the shape of the power input constraints."""
    ...
    def imin_profile(self) -> pd.Series:
        """Returns the Imin profile, which is the minimum power input profile, as a pd.Series. This is used for storage resources to define the shape of the power input constraints."""
    ...
    def scaled_imax_profile(self) -> Dict[int, pd.Series]:
        """Returns the scaled Imax profile, which is the maximum power input profile scaled by the planned storage capacity in each modeled year. This is used for storage resources to define the shape of the power input constraints in each modeled year."""
        return {
    ...
    def scaled_imin_profile(self) -> Dict[int, pd.Series]:
        """Returns the scaled Imin profile, which is the minimum power input profile scaled by the planned storage capacity in each modeled year. This is used for storage resources to define the shape of the power input constraints in each modeled year."""
        return {
    ...
    def scaled_SOC_max_profile(self) -> Dict[int, pd.Series]:
        """Returns the scaled SOC max profile, which is the maximum state of charge profile scaled by the planned storage capacity in each modeled year. This is used for storage resources to define the shape of the state of charge constraints in each modeled year."""
    ...
    def _operational_storage_capacity(self, block, modeled_year: pd.Timestamp):
        """Calculate the operational storage capacity in each modeled year as the sum of the cumulative selected storage capacity
        and the initial storage capacity, minus cumulative retirements."""
    ...
    def _annual_total_investment_cost(self, block: pyo.Block, modeled_year: pd.Timestamp):
        """Total investment costs of the resource in each year, including both capital costs and fixed O&M costs."""
    ...
    def _net_power_output(self, block, modeled_year: pd.Timestamp, dispatch_window, timestamp: pd.Timestamp):
        """Net power output of the storage resource in each timepoint, which is equal to power output minus power input."""
        return (
    ...
    def _power_input_max(self, block, modeled_year: pd.Timestamp, dispatch_window, timestamp: pd.Timestamp):
        """Calculate the maximum power input in a given timepoint based on the resource's operational capacity and Imax profile."""
    ...
    def _power_input_min(self, block, modeled_year: pd.Timestamp, dispatch_window, timestamp: pd.Timestamp):
        """Calculate the minimum power input in a given timepoint based on the resource's operational capacity and Imin profile."""
    ...
    def _power_input_annual(self, block, modeled_year):
        """Calculate the annual power input in each modeled year by summing the power input across all timepoints in that year."""
    ...
    def _soc_intra_anchoring_constraint(self, block, modeled_year, dispatch_window):
        ...
        or when the model is set to loop back around dispatch windows.
        Otherwise, intra-period SOC in the first timestamp of every dispatch window is equal to the initial SOC parameter provided by the user.
        """
    ...
    def _power_input_variable_cost(self, block, modeled_year: pd.Timestamp, dispatch_window, timestamp: pd.Timestamp):
        ...
        return (
            self.formulation_block.power_input[modeled_year, dispatch_window, timestamp]
            * self.variable_cost_power_input.data.at[timestamp.replace(year=modeled_year.year)]
        )
    ...
    def _erm_charging_efficiency(
        ...
        """Calculate the charging efficiency for a given weather timestamp.
        Note: This is redefined on StorageResourceGroup to take the average efficiency across all storage resources in the group.
        """
    ...
    def _erm_discharging_efficiency(
        ...
        """Calculate the discharging efficiency for a given weather timestamp.
        Note: This is redefined on StorageResourceGroup to take the average efficiency across all storage resources in the group.
        """
    ...
    def _erm_net_power_output(
        ...
        """Calculate the net power output for a given weather timestamp, which is equal to power output minus power input."""
        return (
    ...
    def _erm_dispatch_cost(
        ...
        """Calculate the dispatch cost for a given weather timestamp, which is equal to the net power output multiplied by the dispatch cost per MWh."""
        return (
    ...
    def _erm_annual_dispatch_cost(self, block: pyo.Block, modeled_year: pd.Timestamp):
        """Calculate the annual dispatch cost for a given modeled year."""
    ...
    def _erm_soc_max_constraint(
        ...
        """Constrains the state of charge for a given hour to be less than the operational storage capacity scaled by the discharging efficiency."""
        return (
            block.erm_state_of_charge[modeled_year, weather_period, weather_timestamp]
    ...
    def check_if_operationally_equal(self, other):
        """Check if this resource is operationally equal to another resource, meaning that they have the same operational constraints and parameters.
        This is used to determine whether two resources can be grouped together in an operational group."""
        equal = (
...
class StorageResourceGroup(BaseStorageResourceGroup, GenericResourceGroup, StorageResource):
    ...
    def _operational_storage_capacity(self, block, modeled_year: pd.Timestamp):
        """Defines the operational storage capacity for a given modeled year as the sum of the operational storage capacities for all linked Assets."""
        return (
            pyo.quicksum(
    ...
    def _cumulative_selected_storage_capacity(self, block, modeled_year: pd.Timestamp):
        """Defines the cumulative capacity built in the current and previous modeled year(s) as the sum of the selected storage capacities for all linked Assets."""
        return pyo.quicksum(
    ...
    def _selected_storage_capacity(self, block, modeled_year: pd.Timestamp):
        """Defines the capacity built in each modeled year for all linked Assets as:
        - For the first modeled year: the cumulative selected storage capacity in the current year
        - For subsequent modeled years: the difference between the cumulative selected storage capacity in the current year and the previous year.
        """
        model = block.model()
    ...
    def _retired_storage_capacity(self, block, modeled_year: pd.Timestamp):
        """Defines the capacity retired in each modeled year as the sum of the retired storage capacity for all linked Assets."""
        return pyo.quicksum(
    ...
    def _cumulative_retired_storage_capacity(self, block, modeled_year: pd.Timestamp):
        """Defines the cumulative capacity retired in the current and previous modeled year(s) as:
        - For the first modeled year: the retired storage capacity in the current year
        - For subsequent modeled years: the sum of the retired storage capacity in the current year and the cumulative retired storage capacity in the previous year.
        """
        model = block.model()
        sorted_years = sorted(model.MODELED_YEARS)
        year_index = sorted_years.index(modeled_year)
    ...
    def _erm_charging_efficiency(
        ...
        """Defines the charging efficiency for ERM dispatch as the average of the charging efficiencies across all member resources in the group."""
    ...
    def _erm_discharging_efficiency(
        ...
        """Defines the discharging efficiency for ERM dispatch as the average of the discharging efficiencies across all member resources in the group."""
        # Take the average efficiency over all member resources for purposes of ERM dispatch
        return sum(
