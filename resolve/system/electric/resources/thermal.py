...
from kit.system.electric.resources.thermal import BaseThermalUnitCommitmentResourceGroup
...
class ThermalResource(BaseThermalResource, GenericResource):
    """Fuel-burning resource."""

    SAVE_PATH: ClassVar[str] = "resources/thermal"

    _DOCS_URL: ClassVar[str] = (
        "https://docs.ethree.com/projects/kit/en/main/system/electric/resources/thermal.html#resolve.system.thermal.ThermalResource."
    )
    ...
    @property
    def is_multi_fuel(self) -> bool:
        """Boolean property indicating whether the resource has more than one candidate fuel."""
        return len(self.candidate_fuels) > 1
    ...
    def _construct_operational_rules(
        ...
        if self.is_multi_fuel:
            # If multiple candidate fuels, track power output by fuel for proper RPS accounting
            pyomo_components.update(
        ...
        # Annual power output by fuel expression is needed regardless for results reporting and RPS accounting
        pyomo_components.update(
    ...
    def _annual_resource_fuel_consumption_by_fuel(self, block, candidate_fuel: str, modeled_year: pd.Timestamp):
        ...
        Args:
            ...
            candidate_fuel (str): The candidate fuel for which the consumption is being calculated
            modeled_year (pd.Timestamp): The timestamp representing the modeled year

        Returns:
            pyo.Expression. The total fuel consumption of the specified fuel for the modeled year.
        """
    ...
    def _total_resource_fuel_consumption_in_timepoint_mmbtu(
        ...
        Returns:
            pyo.Expression. The total fuel consumption of the resource across all candidate fuels for the specified timepoint.
        """
    ...
    def _annual_total_resource_fuel_consumption_mmbtu(self, block, modeled_year: pd.Timestamp):
        ...
        Returns:
            pyo.Expression. The total annual fuel consumption of the resource across all candidate fuels for the modeled year.
        """
    ...
    def _annual_total_resource_fuel_cost(self, block, modeled_year: pd.Timestamp):
        """Calculate the resource's total annual fuel costs across all candidate fuels flagged as commodities.
        ...
        Returns:
            pyo.Expression. The sum of the resource's candidate fuel costs for the modeled year.
        """
    ...
    def _power_output_by_fuel_constraint(
        ...
        Args:
            block: The block object associated with the constraint
            candidate_fuel (str): The candidate fuel for which the constraint is being evaluated
            modeled_year (pd.Timestamp): The timestamp representing the modeled year
            dispatch_window (pd.Timestamp): The timestamp representing the dispatch window
            timestamp (pd.Timestamp): The timestamp for which the constraint is being evaluated
    ...
    def _annual_power_output_by_fuel(
        ...
        """
        Calculate the resource's total annual power output by fuel.

        Args:
            block: The block object associated with the expression
            candidate_fuel (str): The candidate fuel for which the power output is being calculated
            modeled_year (pd.Timestamp): The timestamp representing the modeled year

         Returns:
            pyo.Expression. The total power output of the resource for the specified fuel for the modeled year.
        """
        if self.is_multi_fuel:
            return block.model().sum_timepoint_component_slice_to_annual(
            ...
            return block.power_output_annual[modeled_year]
...
class ThermalUnitCommitmentResource(BaseThermalUnitCommitmentResource, ThermalResource, UnitCommitmentResource):
    SAVE_PATH: ClassVar[str] = "resources/thermal"
    ...
    start_fuel_use: Annotated[float, Metadata(category=FieldCategory.OPERATIONS)] = Field(
        0,
        description="[UC] Amount of fuel used per unit start [MMBTU/start]",
        title=f"Start Fuel Use. Default 0.",
        ge=0,
    )
    ...
    def _construct_operational_rules(
        ...
        candidate_fuels = pyomo_components["CANDIDATE_FUELS"]

        # Resource fuel consumption and sync cond constraints are applied at the resource level
        pyomo_components.update(
            resource_fuel_consumption_constraint=pyo.Constraint(
                model.MODELED_YEARS,
                model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
        ...
        if self.is_multi_fuel:
            # If multiple candidate fuels, track UC and synchronous condenser by candidate fuel
            pyomo_components.update(
                committed_units_by_fuel=pyo.Var(
                    candidate_fuels,
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    within=pyo.NonNegativeReals,
                    doc="Number of committed units attributed to each candidate fuel",
                ),
                sync_cond_power_input_by_fuel=pyo.Expression(
                    candidate_fuels,
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    rule=self._sync_cond_power_input_by_fuel,
                    doc="Synchronous Condenser Power Input by Fuel (MW)",
                ),
                start_units_by_fuel=pyo.Var(
                    candidate_fuels,
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    within=pyo.NonNegativeReals,
                    doc="Number of starts attributed to each candidate fuel",
                ),
                shutdown_units_by_fuel=pyo.Var(
                    candidate_fuels,
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    within=pyo.NonNegativeReals,
                    doc="Number of shutdowns attributed to each candidate fuel",
                ),
                committed_units_by_fuel_sum_constraint=pyo.Constraint(
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    rule=self._committed_units_by_fuel_sum_constraint,
                ),
                start_units_by_fuel_sum_constraint=pyo.Constraint(
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    rule=self._start_units_by_fuel_sum_constraint,
                ),
                shutdown_units_by_fuel_sum_constraint=pyo.Constraint(
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    rule=self._shutdown_units_by_fuel_sum_constraint,
                ),
                committed_units_by_fuel_tracking_constraint=pyo.Constraint(
                    candidate_fuels,
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    rule=self._committed_units_by_fuel_tracking_constraint,
                ),
                power_output_by_fuel_constraint=pyo.Constraint(
                    candidate_fuels,
                    model.MODELED_YEARS,
                ...
                power_output_by_fuel_max_constraint=pyo.Constraint(
                    candidate_fuels,
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    rule=self._power_output_by_fuel_max_constraint,
                ),
                power_output_by_fuel_min_constraint=pyo.Constraint(
                    candidate_fuels,
                    model.MODELED_YEARS,
                    model.DISPATCH_WINDOWS_AND_TIMESTAMPS,
                    rule=self._power_output_by_fuel_min_constraint,
                ),
            )
            if (
                ...
                pyomo_components.update(
                    committed_units_by_fuel_tracking_inter_period_constraint=pyo.Constraint(
                        candidate_fuels,
                        model.MODELED_YEARS,
                        model.CHRONO_PERIODS,
                        rule=self._committed_units_by_fuel_tracking_inter_period_constraint,
                    )
                )

        # Annual sync cond power input is the same regardless of candidate fuel tracking
        pyomo_components.update(
            annual_sync_cond_power_input_by_fuel=pyo.Expression(
                candidate_fuels,
                model.MODELED_YEARS,
                rule=self._annual_sync_cond_power_input_by_fuel,
    ...
    def _synchronous_condenser_constraint(
        ...
        """
        Constrain synchronous condenser power input from committed capacity.
        Args:
            block: The block object associated with the constraint.
            modeled_year (pd.Timestamp): The timestamp representing the modeled year.
            dispatch_window (pd.Timestamp): The timestamp representing the dispatch window.
            timestamp (pd.Timestamp): The timestamp for which the constraint is being evaluated.
        Returns:
            pyo.Constraint. The sync condenser power input equals the addition_to_load * committed capacity.
        """
        return block.sync_cond_power_input[modeled_year, dispatch_window, timestamp] == (
    ...
    def _commitment_tracking_constraint(
        ...
        """If multiple fuels, skip aggregate tracking; fuel-level tracking plus sum constraints imply it."""
        if self.is_multi_fuel:
            return pyo.Constraint.Skip
        else:
            return super()._commitment_tracking_constraint(block, modeled_year, dispatch_window, timestamp)

    def _commitment_tracking_inter_period_constraint(self, block, modeled_year: pd.Timestamp, chrono_period):
        """If multiple fuels, skip aggregate inter-period tracking; fuel-level tracking plus sum constraints imply it."""
        if self.is_multi_fuel:
            return pyo.Constraint.Skip
        else:
            return super()._commitment_tracking_inter_period_constraint(block, modeled_year, chrono_period)

    def _committed_units_by_fuel_sum_constraint(
        self, block: pyo.Block, modeled_year: pd.Timestamp, dispatch_window: pd.Timestamp, timestamp: pd.Timestamp
    ):
        """Constrain total committed units to equal committed units by fuel.

        Args:
            block: Pyomo block containing unit commitment variables.
            modeled_year: Modeled year for the constraint.
            dispatch_window: Dispatch window containing the timestamp.
            timestamp: Timestamp for the constraint.

        Returns:
            Pyomo constraint expression tying total committed units to the sum by fuel.
        """
        return block.committed_units[modeled_year, dispatch_window, timestamp] == sum(
            block.committed_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp]
            for candidate_fuel in self.candidate_fuels
        )

    def _start_units_by_fuel_sum_constraint(
        self, block: pyo.Block, modeled_year: pd.Timestamp, dispatch_window: pd.Timestamp, timestamp: pd.Timestamp
    ):
        """Constrain total starts to equal starts by fuel.
        ...
        Returns:
            Pyomo constraint expression tying total starts to the sum by fuel.
        """
        return block.start_units[modeled_year, dispatch_window, timestamp] == sum(
            block.start_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp]
            for candidate_fuel in self.candidate_fuels
        )

    def _shutdown_units_by_fuel_sum_constraint(
        self, block: pyo.Block, modeled_year: pd.Timestamp, dispatch_window: pd.Timestamp, timestamp: pd.Timestamp
    ):
        """Constrain total shutdowns to equal shutdowns by fuel.
        ...
        Returns:
            Pyomo constraint expression tying total shutdowns to the sum by fuel.
        """
        return block.shutdown_units[modeled_year, dispatch_window, timestamp] == sum(
            block.shutdown_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp]
            for candidate_fuel in self.candidate_fuels
        )

    def _committed_units_by_fuel_tracking_constraint(
        self,
        block: pyo.Block,
        candidate_fuel: str,
        modeled_year: pd.Timestamp,
        ...
        """Track committed units by fuel between adjacent dispatch-window timestamps.

        Args:
            block: Pyomo block containing unit commitment variables.
            candidate_fuel: Candidate fuel whose committed units are tracked.
            modeled_year: Modeled year for the constraint.
            dispatch_window: Dispatch window containing the timestamp.
            timestamp: Timestamp preceding the tracked next timestamp.

        Returns:
            Pyomo constraint expression, or ``pyo.Constraint.Skip`` at inter-period boundaries.
        """
        if (
        ...
        return (
            block.committed_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, next_timestamp]
            == block.committed_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp]
            + block.start_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, next_timestamp]
            - block.shutdown_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, next_timestamp]
        )

    def _committed_units_by_fuel_tracking_inter_period_constraint(
        self,
        block: pyo.Block,
        candidate_fuel: str,
        modeled_year: pd.Timestamp,
        ...
        """Track committed units by fuel across connected chronological periods.

        Args:
            block: Pyomo block containing unit commitment variables.
            candidate_fuel: Candidate fuel whose committed units are tracked.
            modeled_year: Modeled year for the constraint.
            chrono_period: Chronological period whose boundary timepoints are linked.

        Returns:
            Pyomo constraint expression linking commitment state across periods.
        """
        ...
        return (
            block.committed_units_by_fuel[candidate_fuel, chrono_period_2_index]
            == block.committed_units_by_fuel[candidate_fuel, chrono_period_1_index]
            + block.start_units_by_fuel[candidate_fuel, chrono_period_2_index]
            - block.shutdown_units_by_fuel[candidate_fuel, chrono_period_2_index]
        )
    ...
    def _power_output_by_fuel_constraint(
        ...
        start_fuel_use = (
            block.start_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp] * self.start_fuel_use
        )
    ...
    def _total_power_output_by_fuel_constraint(
        ...
        Returns:
            pyo.Constraint. The sum of power output of a resource's candidate fuels == the resource's total power output
        """
    ...
    def _power_output_by_fuel_max_constraint(
        self,
        block: pyo.Block,
        candidate_fuel: str,
        modeled_year: pd.Timestamp,
        ...
        """Limit fuel-specific power output by committed units and maximum output.

        Args:
            block: Pyomo block containing fuel-specific dispatch variables.
            candidate_fuel: Candidate fuel whose output is constrained.
            modeled_year: Modeled year for the constraint.
            dispatch_window: Dispatch window containing the timestamp.
            timestamp: Timestamp for the constraint.

        Returns:
            Pyomo constraint expression for the fuel-specific maximum output.
        """
        return block.power_output_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp] <= (
            block.unit_size[modeled_year]
            * block.committed_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp]
            * self.power_output_max.data.at[timestamp]
        )

    def _power_output_by_fuel_min_constraint(
        self,
        block: pyo.Block,
        candidate_fuel: str,
        modeled_year: pd.Timestamp,
        ...
        """Enforce fuel-specific minimum output for committed units.
        ...
        Returns:
            Pyomo constraint expression for the fuel-specific minimum output.
        """
        return block.power_output_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp] >= (
            block.unit_size[modeled_year]
            * block.committed_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp]
            * max(self.min_stable_level, self.power_output_min.data.at[timestamp])
        )

    def _synchronous_condenser_by_fuel_constraint(
        self,
        block,
        modeled_year: pd.Timestamp,
        dispatch_window,
        ...
        """
        Constrain synchronous condenser power input to equal the sum of fuel-specific contributions.
        ...
        Returns:
            pyo.Constraint. The synchronous condenser power input equals the sum of fuel-specific contributions.
        """
        return block.sync_cond_power_input[modeled_year, dispatch_window, timestamp] == sum(
            block.sync_cond_power_input_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp]
            for candidate_fuel in self.candidate_fuels
        )

    def _sync_cond_power_input_by_fuel(
        self,
        block,
        candidate_fuel: str,
        modeled_year: pd.Timestamp,
        ...
        """Calculate synchronous condenser input for a candidate fuel.

        Args:
            block: Pyomo block containing synchronous condenser variables.
            candidate_fuel: Candidate fuel whose synchronous condenser input is calculated.
            modeled_year: Modeled year for the expression.
            dispatch_window: Dispatch window containing timestamp.
            timestamp: Timestamp for the expression.

        Returns:
            Pyomo expression for fuel-specific synchronous condenser input.
        """
        return (
            self.addition_to_load
            * block.unit_size[modeled_year]
            * block.committed_units_by_fuel[candidate_fuel, modeled_year, dispatch_window, timestamp]
        )

    def _annual_sync_cond_power_input_by_fuel(self, block, candidate_fuel: str, modeled_year: pd.Timestamp):
        """Calculate annual synchronous condenser input for a candidate fuel.

        Args:
            block: Pyomo block containing synchronous condenser variables.
            candidate_fuel: Candidate fuel whose annual input is calculated.
            modeled_year: Modeled year for the expression.

        Returns:
            Annual synchronous condenser power input for the candidate fuel.
        """
        if self.is_multi_fuel:
            return block.model().sum_timepoint_component_slice_to_annual(
                block.sync_cond_power_input_by_fuel[candidate_fuel, modeled_year, :, :]
            )
        else:
            return block.model().sum_timepoint_component_slice_to_annual(
                block.sync_cond_power_input[modeled_year, :, :]
            )
    ...
    def _annual_total_operational_cost(self, block, modeled_year: pd.Timestamp):
        """
        The total annual operational costs of the resource. This expression is updated from the definition in GenericResource to include the resource_fuel_cost and start fuel use.
        ...
        Returns:
            pyo.Expression. The total annual operational cost of the resource, including fuel costs and startup fuel use.
        """
        total_operational_cost = (
...
class ThermalResourceGroup(BaseThermalResourceGroup, GenericResourceGroup, ThermalResource):
...
class ThermalUnitCommitmentResourceGroup(
    BaseThermalUnitCommitmentResourceGroup, UnitCommitmentResourceGroup, ThermalUnitCommitmentResource
):
    SAVE_PATH: ClassVar[str] = "resources/thermal/groups"
