...
class UnitCommitmentResource(BaseUnitCommitmentResource, GenericResource, ABC):
    """Resource that can be committed in discrete units.

    The `unit_commitment_mode` attribute controls whether the commitment is represented as
    an integer count of units, a linear variable, or a single binary variable."""
    ...
    start_cost: Annotated[ts.NumericTimeseries, Metadata(category=FieldCategory.OPERATIONS)] = Field(
    ...
    shutdown_cost: Annotated[ts.NumericTimeseries, Metadata(category=FieldCategory.OPERATIONS)] = Field(
        default_factory=ts.NumericTimeseries.zero,
    ...
    @field_validator("start_cost", "shutdown_cost", mode="before")
    @classmethod
    def convert_scalar_commitment_cost_to_timeseries(cls, value, info: ValidationInfo):
        """Support legacy scalar start/shutdown costs as modeled-year timeseries defaults."""
        if value is None or isinstance(value, ts.NumericTimeseries):
            return value
        ...
        return ts.NumericTimeseries(
            ...
            data=pd.Series({pd.Timestamp("1/1/1900"): scalar_value}),
        )

    @field_validator("start_cost", "shutdown_cost")
    @classmethod
    def commitment_cost_must_be_nonnegative(cls, value: ts.NumericTimeseries, info: ValidationInfo):
        if (value.data < 0).any():
            invalid_values = value.data[value.data < 0]
            raise ValueError(f"Values for {info.field_name} must be non-negative, see values: \n{invalid_values}")
    ...
    def _construct_operational_rules(
        ...
        # Ensure that the min power output constraint is re-constructed if a min stable level is defined
    ...
    def _start_cost_in_timepoint(
        ...
        return (
            self.formulation_block.start_units[modeled_year, dispatch_window, timestamp]
            * self.start_cost.data.at[timestamp.replace(year=modeled_year.year)]
        )
    ...
    def _shutdown_cost_in_timepoint(
        ...
        return (
            self.formulation_block.shutdown_units[modeled_year, dispatch_window, timestamp]
            * self.shutdown_cost.data.at[timestamp.replace(year=modeled_year.year)]
        )
...
class UnitCommitmentResourceGroup(BaseUnitCommitmentResourceGroup, GenericResourceGroup, UnitCommitmentResource):
