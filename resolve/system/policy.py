...
class AnnualEnergyStandard(Policy):
    ...
    def _energy_policy_annual_contribution_by_resource(self, block, resource, modeled_year):
        ...
        if (
            isinstance(resource_instance, ThermalResource)
            and len(set(resource_instance.candidate_fuels.keys()).intersection(set(self.candidate_fuels.keys()))) > 0
            and resource_instance.has_operational_rules
        ):
            eligible_candidate_fuels = set(resource_instance.candidate_fuels.keys()).intersection(
                set(self.candidate_fuels.keys())
            )
            resource_multiplier = self.resources[resource].multiplier.data.at[modeled_year]
            thermal_contributions = sum(
                resource_multiplier
                * resource_instance.formulation_block.annual_power_output_by_fuel[candidate_fuel, modeled_year]
                for candidate_fuel in eligible_candidate_fuels
            )
            # Subtract any synchronous condenser addition to load aligned to the eligible candidate fuel(s).
            if isinstance(resource_instance, ThermalUnitCommitmentResource):
                thermal_contributions -= sum(
                    resource_multiplier
                    * resource_instance.formulation_block.annual_sync_cond_power_input_by_fuel[
                        candidate_fuel, modeled_year
                    ]
                    for candidate_fuel in eligible_candidate_fuels
                )
            return thermal_contributions
        # Contributions from other resources (e.g., solar, wind)
        elif resource_instance.has_operational_rules:
...
class EnergyReserveMargin(Policy):
    ...
    def _construct_investment_rules(
        ...
        assets = pyo.Set(initialize=self.assets.keys())
        pyomo_components.update(
            ...
            tx_path_contribution=pyo.Var(  # tx path is a var to allow for CC based on hourly availability from neighboring zones
                tx_paths,
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
            ...
            tx_path_contribution_constraint=pyo.Constraint(
                tx_paths,
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
                rule=self._tx_path_contribution_constraint,
            ),
            ...
            hourly_storage_contribution=pyo.Expression(
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
                rule=self._hourly_storage_contribution,
                doc="Contributions from Storage (MW)",
            ),
            hourly_shed_dr_contribution=pyo.Expression(
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
                rule=self._hourly_shed_dr_contribution,
                doc="Contributions from Shed DR (MW)",
            ),
            hourly_other_resource_contribution=pyo.Expression(
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
                rule=self._hourly_other_resource_contribution,
                doc="Contributions from other Resources (MW)",
            ),
            hourly_generic_asset_contribution=pyo.Expression(
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
                rule=self._hourly_generic_asset_contribution,
                doc="Contributions from Generic Assets (MW)",
            ),
            hourly_tx_path_contribution=pyo.Expression(
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
                rule=self._hourly_tx_path_contribution,
                doc="Contributions from Tx Paths (MW)",
            ),
            hourly_contribution_by_asset=pyo.Expression(
                assets,
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
                rule=self._hourly_contribution_by_asset,
                doc="Total ERM Contribution (MW)",
            ),
            policy_lhs=pyo.Expression(
                model.MODELED_YEARS,
                model.WEATHER_PERIODS_AND_WEATHER_TIMESTAMPS,
    ...
    def _tx_path_contribution_constraint(
        self,
        block: pyo.Block,
        tx_path_name: str,
        modeled_year: pd.Timestamp,
    ...
    def _hourly_storage_contribution(
        self, block, modeled_year: pd.Timestamp, weather_period: pd.Timestamp, weather_timestamp: pd.Timestamp
    ):
        """All contributions from storage resources for a weather timestamp"""
        return pyo.quicksum(
    ...
    def _hourly_shed_dr_contribution(
        self, block, modeled_year: pd.Timestamp, weather_period: pd.Timestamp, weather_timestamp: pd.Timestamp
    ):
        """All contributions from shed DR resources for a weather timestamp"""
        return pyo.quicksum(
    ...
    def _hourly_other_resource_contribution(
        self, block, modeled_year: pd.Timestamp, weather_period: pd.Timestamp, weather_timestamp: pd.Timestamp
    ):
        """All contributions from other resources for a weather timestamp"""
        return pyo.quicksum(
    ...
    def _hourly_tx_path_contribution(
        self, block, modeled_year: pd.Timestamp, weather_period: pd.Timestamp, weather_timestamp: pd.Timestamp
    ):
        """All contributions from tx paths for a weather timestamp"""
        return pyo.quicksum(
            block.tx_path_contribution[tx_path_name, modeled_year, weather_period, weather_timestamp]
            for tx_path_name in self.tx_paths.keys()
        )

    def _hourly_generic_asset_contribution(
        self, block, modeled_year: pd.Timestamp, weather_period: pd.Timestamp, weather_timestamp: pd.Timestamp
    ):
        """All contributions from generic assets for a weather timestamp"""
        return pyo.quicksum(
    ...
    def _hourly_contribution_by_asset(
        self,
        block: pyo.Block,
        asset_name: str,
        modeled_year: pd.Timestamp,
        ...
        """Create a single expression for each asset's hourly contribution (mostly for results reporting)"""
        if asset_name in self.storage_resources:
            return block.storage_resource_contribution[asset_name, modeled_year, weather_period, weather_timestamp]
        elif asset_name in self.shed_dr_resources:
            return block.shed_dr_resource_contribution[asset_name, modeled_year, weather_period, weather_timestamp]
        elif asset_name in self.other_resources:
            return block.other_resource_contribution[asset_name, modeled_year, weather_period, weather_timestamp]
        elif asset_name in self.tx_paths:
            return block.tx_path_contribution[asset_name, modeled_year, weather_period, weather_timestamp]
        elif asset_name in self.assets_:
            return block.generic_asset_contribution[asset_name, modeled_year, weather_period, weather_timestamp]
        else:
            raise ValueError(f"Asset {asset_name} not found in any ERM asset categories.")

    def _policy_lhs(
        self, block, modeled_year: pd.Timestamp, weather_period: pd.Timestamp, weather_timestamp: pd.Timestamp
    ):
        total_storage_contribution = block.hourly_storage_contribution[modeled_year, weather_period, weather_timestamp]
        total_shed_dr_contribution = block.hourly_shed_dr_contribution[modeled_year, weather_period, weather_timestamp]
        total_other_resource_contribution = block.hourly_other_resource_contribution[
            modeled_year, weather_period, weather_timestamp
        ]
        total_tx_path_contribution = block.hourly_tx_path_contribution[modeled_year, weather_period, weather_timestamp]
        total_generic_asset_contribution = block.hourly_generic_asset_contribution[
            modeled_year, weather_period, weather_timestamp
        ]
        ...
        return (
            ...
            * pyo.quicksum(
                block.model().sum_weather_timestamp_component_slice_to_annual(
                    block.tx_path_contribution[tx_path, modeled_year, :, :]
                )
                for tx_path in self.tx_paths.keys()
            )  # subtract small value for tx path contributions so it maxes out this contribution
        )
    ...
    def _reorder_weather_timestamp_results_columns(self, results_df: pd.DataFrame) -> pd.DataFrame:
        """Reorder weather timestamp results columns so that first four columns are Achieved, Target, Slack, Dual."""
        priority_cols = ["Slack (MW)", "Achieved (MW)", "Target (MW)", "Unweighted Dual Value ($/Unit)"]
        existing_cols = [col for col in priority_cols if col in results_df.columns]
        if not existing_cols:
            return results_df  # If expected columns are not there, return as-is
        remaining_cols = [col for col in results_df.columns if col not in existing_cols]
        return results_df.loc[:, existing_cols + remaining_cols]
