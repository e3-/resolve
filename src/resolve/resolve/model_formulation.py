...
from resolve.common import temporal
from resolve.common.asset.plant import ResourceCategory
from resolve.common.policy import ConstraintOperator
from resolve.core.component import Component
from resolve.core.linkage import DeliverabilityStatus
...
from resolve.core.utils.pandas_utils import convert_index_levels_to_datetime
...
from resolve.resolve import settings


# TODO (RG): Figure out how to align Pyomo's ureg with resolve.ureg
...
class ResolveCase(Component):
    ...
    def update_resource_profiles(self):
        ...
        for resource, obj in tqdm(
            ...
            if "Solar" not in resource and "Wind" not in resource:
                continue
    ...
    def _construct_model(self):
        ...
        # Expression used for assets, not just zonal balance
        # TODO: Reduce `Asset_New_Power_MW`, because assets cannot create power, only plants + transmission paths
        @mark_pyomo_component
        @self.model.Expression(self.model.ASSETS, self.model.TIMEPOINTS)
        def Asset_Net_Power_MW(model, asset, model_year, rep_period, hour):
            MW_power = MW_load = 0  # instantiate
            if asset in model.PLANTS_THAT_PROVIDE_POWER:
                MW_power = model.Provide_Power_MW[asset, model_year, rep_period, hour]
            elif asset in model.TRANSMISSION_LINES:
                MW_power = model.Transmit_Power_MW[asset, model_year, rep_period, hour]

            if asset in model.PLANTS_THAT_INCREASE_LOAD:
                MW_load = model.Increase_Load_MW[asset, model_year, rep_period, hour]

            return MW_power - MW_load
        ...
        # def Total_Cost(model):
        #     return sum(
        #         model.Reliability_Capacity_In_Model_Year["System RA", resource, model_year]
        #         for resource in model.RESOURCES if "Solar" in resource
        #         for model_year in model.MODEL_YEARS
        #     )
