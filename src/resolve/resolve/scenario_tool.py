...
def save_multiple_cases(sheet_name: str, model: str = "resolve", data_folder: Optional[os.PathLike] = None):
    wb = xw.Book.caller()
    ...
    for case in sheet.range("cases_to_save").value:
        if case is not None:
            ...
            wb.app.calculate()
            save_RESOLVE_case_settings(sheet_name=sheet_name, data_folder=data_folder)


def save_RESOLVE_case_settings(sheet_name: str, model: str = "resolve", data_folder: Optional[os.PathLike] = None):
    ...
    if sheet.range("settings_rep_period_method").value == "manual":
        save_manual_temporal_settings(settings_name=settings_name, data_folder=data_folder)

    # Save extras (hydro params and simultaneous flow limits)
    wb.app.status_bar = f"{settings_name}: extras"
    save_extras(settings_name=settings_name, data_folder=data_folder)

    # Save custom constraints
    wb.app.status_bar = f"{settings_name}: custom constraints"
    save_custom_constraints(settings_name=settings_name, data_folder=data_folder)

    wb.app.status_bar = None


def save_extras(settings_name: str, data_folder: Optional[os.PathLike] = None):
...
def save_manual_temporal_settings(settings_name: str, data_folder: Optional[os.PathLike] = None):
...
def save_custom_constraints(settings_name: str, data_folder: Optional[os.PathLike] = None):
...
def save_custom_timepoint_constraints(settings_name: str, data_folder: Optional[os.PathLike] = None):
...
def load_RESOLVE_case_settings(sheet_name: str, model: str = "resolve", data_folder: Optional[os.PathLike] = None):
