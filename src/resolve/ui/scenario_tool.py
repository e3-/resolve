...
def load_system(sheet_name: str, wb: Optional["Book"] = None, data_folder: Optional[os.PathLike] = None):
    ...
    data_folder = upath.UPath(data_folder) / "interim"
    system_dir = data_folder / "interim" / "systems" / sheet.range("system_to_load").value

    # Load existing system component and linkages
    df_components = pd.read_csv(system_dir / "components.csv")
    df_linkages = pd.read_csv(system_dir / "linkages.csv")
    df_attributes = pd.read_csv(system_dir / "attributes.csv")

    # Populate data into UI
    sheet.range("component_types").clear_contents()
    sheet.range("component_names").clear_contents()
    sheet.range("linkage_types").clear_contents()
    sheet.range("linkage_instance_from").clear_contents()
    sheet.range("linkage_instance_to").clear_contents()
    ...
    sheet.range("linkage_types").value = df_linkages["linkage"].values.reshape(-1, 1)
    sheet.range("linkage_instance_from").value = df_linkages["component_from"].values.reshape(-1, 1)
    sheet.range("linkage_instance_to").value = df_linkages["component_to"].values.reshape(-1, 1)
